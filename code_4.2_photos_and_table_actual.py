...
while (current_waypoint < get_non_zero_rows(waypoints)):
    ...
    while ((position_remaining > 5) or (abs(alt_remaining)> 1)):
        ...
        print("Dist:",round(position_remaining,2),"	Alt:",abs(round(alt_remaining,2)),"	To:",waypoints[current_waypoint][0]," Mode:",waypoints[current_waypoint][1])    
    ...
    logging.info("waypont reached: %s", current_waypoint+1)
    	...
    	try:
    		...
    		for i in range(0,result_and_image_info.shape[0]):
    			if int(input_number) == result_and_image_info[i][0]:
    				...
    				time.sleep(1)
    				new_way_point = np.array([current_waypoint+ 1,1,result_and_image_info[i][1],result_and_image_info[i][2],result_and_image_info[i][3]])
    				waypoints = np.insert(waypoints,current_waypoint + 1, new_way_point,0)
    				print(waypoints)
    				current_waypoint = current_waypoint +1
